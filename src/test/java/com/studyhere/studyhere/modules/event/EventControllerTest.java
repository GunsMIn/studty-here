...
class EventControllerTest extends AbstractContainerBaseTest {
    ...
    void newEnrollment_to_FCFS_event_accepted() throws Exception {
        Account whiteship = accountFactory.createAccount("gunwoo");
    ...
    void accepted_account_cancelEnrollment_to_FCFS_event_not_accepted() throws Exception {
        Account gunwoo = accountRepository.findByNickname("gunwoo");

        Account may = accountFactory.createAccount("may");
        Study study = studyFactory.createStudy("test-study", gunwoo);
        Event event = createEvent("test-event", EventType.FCFS, 2, study, gunwoo);

        eventService.newEnrollment(event, may);
        eventService.newEnrollment(event, gunwoo);
        eventService.newEnrollment(event, gunwoo);

        isAccepted(may, event);
        isAccepted(gunwoo, event);
        isNotAccepted(gunwoo, event);
    ...
    void newEnrollment_to_CONFIMATIVE_event_not_accepted() throws Exception {
        ...
        Account keesun = accountRepository.findByNickname("gunwoo");
        isNotAccepted(keesun, event);
    }
